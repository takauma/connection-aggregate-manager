 ...
 */
class BoundRegistRequest extends FormRequest {
	/**
	 * リクエストの認可有無を返却します.
	 * @return bool 認可有無.
	 */
	...
	{
		return [
			"boundName" => "required|max:32|unique:bound_mst,bound_name",
			"vpsName" => "required|max:32|unique:bound_mst,vps_name",
			"vpsKey" => "required|max:128|unique:bound_mst,vps_key",
			"vpsHost" => "required|max:64|unique:bound_mst,vps_host",
			"omrVersion" => "max:16",
			"configVersion" => "max:8",
